...
function ClickableMapComponent(props: ClickableMapInfo) {
  const [initialPosition, setInitialPosition] = useState<[number, number]>([
    props.defaultCoord.lat,
    props.defaultCoord.lon,
  ])
  ...
  const Markers = () => {
    ...
    return selectedPosition ? (
      <Marker key={selectedPosition[0]} position={selectedPosition} interactive={false} />
    ) : null
  }
  ...
  return (
    <MapContainer center={selectedPosition || initialPosition} zoom={12}>
      ...
      {props.eventsPositions?.map((item) => (
        <Marker key={1} position={[item.lat, item.lon]} interactive={false} />
      ))}

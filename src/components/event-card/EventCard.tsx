  ...
  return (
    <FlexboxGrid className={'event-card-root'} key={props.id}>
      <div style={{ padding: '17px' }}>
        ...
        >
          {props.pictureLinks.map((link) => {
            return <img src={link} className={'event-card-img'} alt={`${props.title}.img`} />
          })}
        </Carousel>
      </div>
      <div style={{ marginBottom: '10px' }}>
        ...
        <div
          className={'event-card-second-text text-primary-color'}
      ...
      <FlexboxGrid justify={'space-between'} style={{ flexDirection: 'row', width: '100vw' }}>
        <button className={'event-card-button'} onClick={openMoreInfo}>
          Подробнее
        </button>

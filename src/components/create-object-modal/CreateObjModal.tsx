...
export default function CreateObjModal(props: CreateObjModalProps) {
  ...
  const uploadingHandler = () => {
    ...
    CreateObject(info, radioValue as ObjectTypes)
      .then((res) => {
        const form = new FormData()
        images.forEach((img) => {
          if (img.blobFile !== undefined) form.append('files', img.blobFile)
        })

        UploadObjectImg(form, res.data.result.id, radioValue as ObjectTypes)
          .then(() => {
            props.onClose()
            setTimeout(() => {
              setUploading(false)
